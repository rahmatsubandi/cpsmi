...
class Template
{
     ...
     * @return  void
     */
    function set_master_template($filename = 'index.php') {
        if (file_exists(APPPATH . 'theme_layouts/' . $filename) or file_exists(APPPATH . 'theme_layouts/' . $filename . '.php')) {
            $this->master = $filename;
        } else {
            show_error('The filename provided does not exist in <strong>' . APPPATH . 'themes</strong>. Remember to include the extension if other than ".php"');
        }
    }
    ...
    function initialize($props) {
        ...
        if (isset($props['template'])
                && (file_exists(APPPATH . 'theme_layouts/' . $props['template']) or file_exists(APPPATH . 'theme_layouts/' . $props['template'] . '.php'))) {
            $this->master = $props['template'];
        } else {
            // Master template must exist. Throw error.
            show_error('Either you have not provided a master template or the one provided does not exist in <strong>' . APPPATH . 'themes</strong>. Remember to include the extension if other than ".php"');
        }
     ...
     * @return    void
     */
    function set($region, $content, $overwrite = FALSE) {
    ...
    /**
     * Write content from a View to a region. 'themes within themes'
     *
     * @access    public
     * @param    string    region to write to
    ...
    function load_view($view, $data = NULL, $overwrite = FALSE) {
        ...
        // Get rid of non-themes
        unset($args[0], $args[2], $args[3]);

        // Do we have more view suggestions?
        if (count($args) > 1) {
            foreach ($args as $suggestion) {
                if (file_exists(APPPATH . 'theme_layouts/' . $suggestion . '.php') or file_exists(APPPATH . 'theme_layouts/' . $suggestion)) {
        ...
        // Get menu recursive from './AppMenu'
        $AppMenu = new AppMenu();
        $data['menus'] = $AppMenu->getAll();

        $content = $this->CI->load->view($view, $data, TRUE);
        $this->set('content', $content, $overwrite);
    }
    ...
    function parse_view($region, $view, $data = NULL, $overwrite = FALSE) {
        ...
        $this->set($region, $content, $overwrite);
    }
